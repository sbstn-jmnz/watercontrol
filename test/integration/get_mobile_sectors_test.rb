...
class GetMobileSectorsTest < ActionDispatch::IntegrationTest
  ...
  test 'Debe obtener una lista de sectores con su parcelas, medidores y medicion pendiente' do

    get "/mobileapp/users/#{@user.id}/sectors",{},create_headers

    assert_equal 200, response.status
    assert_equal json(response.body)[:sectors][0].has_key?(:plots), true, "No Plots"
    assert_equal json(response.body)[:sectors][0][:plots][0].has_key?(:meters), true, "No Meters"
    assert_equal json(response.body)[:sectors][0][:plots][0][:meters][0].has_key?(:measure), true, "No Measures"
    assert_equal json(response.body)[:sectors][0][:plots][0][:meters][0][:measure][:status], Measure::STATUS.first, "Debe ser pendiente"
  end
